...
/// Query current output tensor attributes after dynamic-shape selection.
pub struct CurrentOutputAttr {
    pub(crate) inner: rknn_tensor_attr,
}

impl QueryWithInput for CurrentOutputAttr {
    const QUERY_TYPE: rknpu2_sys::_rknn_query_cmd::Type =
        rknpu2_sys::_rknn_query_cmd::RKNN_QUERY_CURRENT_OUTPUT_ATTR;
...
impl From<rknn_tensor_attr> for CurrentOutputAttr {
    fn from(attr: rknn_tensor_attr) -> Self {
        Self { inner: attr }
    }
}

impl TensorAttrView for CurrentOutputAttr {
    fn io(&self) -> Io {
        Io::Output
    }
